...
@Route(value = "service/car-rental-per-month", layout = MainLayout.class)
@RolesAllowed("ADMIN")
public class RentalsPerMonth extends VerticalLayout {
     private final ChartPresenter presenter;

     public RentalsPerMonth(ChartPresenter presenter) {
    ...
    private void configureAxes(Configuration conf) {
        ...
        x.setTitle("Date");
        x.getLabels().setFormat("{value:%b %Y}");
     ...
     private DataSeries createSeries() {
         ...
         presenter.getRentalsPerMonthData()
